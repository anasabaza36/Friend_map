...
async function onSubmit(): Promise<void> {
  ...
  try {
    ...
    toast.error(authStore.error ?? t('login.failed'));
  }
}
</script>
...
<template>
  <AuthLayout>
    ...
    <h2 class="headline">
      ...
      <span class="grad">{{ t('login.titleLine2') }}</span>
    </h2>
    ...
    <form @submit.prevent="onSubmit" class="auth-form">
      ...
      <div class="row-between">
        ...
        <a class="forgot" href="#" @click.prevent>{{ t('login.forgotPassword') }}</a>
      </div>
    ...
    <p class="switch-auth">
      {{ t('login.noAccount') }}
      <router-link to="/register" class="switch-link">{{ t('login.registerLink') }}</router-link>
    </p>
  </AuthLayout>
</template>
